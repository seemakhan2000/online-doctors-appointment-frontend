...
export default function SkinCareDoctorsPage() {
  ...
  useEffect(() => {
    fetchSkinCareDoctors();
  }, []);

  async function fetchSkinCareDoctors() {
    try {
      ...
      const skinDoctors = data.filter(
        (doc: Doctor) =>
          doc.specialization?.toLowerCase().includes("skin care")
      );

      console.log("Filtered skin care doctors:", skinDoctors);
  ...
  return (
    <>
      ...
      <main className="container mt-5 pt-5">
        <h2 className="mb-4">Skin Care Doctors</h2>
